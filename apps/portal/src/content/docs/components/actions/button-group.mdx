...
<Aside type="tip" title="When to use ButtonGroup">
  Use `ButtonGroup` to join related actions into one visually-connected control — a set
  of segmented choices, related toolbar actions. To lay out a primary plus secondary
  button (e.g. a dialog footer), use `ButtonLayout`. For a default action with related
