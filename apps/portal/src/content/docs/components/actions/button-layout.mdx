...
<Aside type="tip" title="When to use ButtonLayout">
  Use `ButtonLayout` to position a primary and secondary action together with correct
  spacing and order — a dialog or drawer footer, a form's submit row. To join segmented
  related actions into one control, use `ButtonGroup`.
</Aside>
