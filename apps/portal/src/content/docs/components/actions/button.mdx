...
<Aside type="tip" title="When to use Button">
  Use `Button` to trigger an action. Keep one filled `primary` per view and demote the
  rest to secondary, outline, or ghost. For navigation between pages, use `Link`. For a
  menu of commands, use `DropdownMenu`. For a primary action with closely-related
  variants, use `SplitButton`.
</Aside>
