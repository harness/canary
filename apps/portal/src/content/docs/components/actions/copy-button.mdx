...
<Aside type="tip" title="When to use CopyButton">
  Use `CopyButton` to copy a value to the clipboard — a connector ID, a commit SHA, a
  generated token — with built-in copied feedback. For a copyable value shown as a
  labeled chip, use `CopyTag`. For a general action, use `Button`.
</Aside>
