...
The `AlertDialog` component is a specialized variant of `ModalDialog` designed for scenarios where users need to confirm or cancel potentially destructive or irreversible actions. It provides a consistent and accessible interface with built-in support for different themes and loading states.
