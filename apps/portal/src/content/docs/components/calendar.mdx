---
title: Calendar
description: Calendar component documentation
---

The `Calendar` component provides a flexible UI element for displaying a date picker. It includes several customizations for styling and functionality.

import { DocsPage } from "@/components/docs-page";

<DocsPage.ComponentExample
  client:only
  code={`() => {
  const [selectedDate, setSelectedDate] = React.useState<Date | undefined>([new Date()]);

  return (
    <Calendar
      mode="range"
      selected={selectedDate}
      onSelect={setSelectedDate}
      showOutsideDays
      fromDate={new Date(2023, 10, 12)}
      toDate={new Date(2023, 11, 15)}
      weekStartsOn={1}
    />
  );
};
  `}
/>

## Usage

```typescript jsx
import { useState } from 'react';
import { Calendar } from '@harnessio/ui/components';

const CalendarExample = () => {
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(new Date());

  return (
    <Calendar
      mode="single"
      selected={selectedDate}
      onSelect={setSelectedDate}
      showOutsideDays={true}
      className="custom-calendar"
      classNames={{
        day: 'custom-day',
        selected: 'custom-selected',
        today: 'custom-today',
      }}
      components={{
        Day: ({ date, selected, ...props }) => (
          <div {...props} className={selected ? 'custom-selected' : 'custom-day'}>
            {date.getDate()}
          </div>
        ),
      }}
      disabled={{ from: new Date(2023, 0, 1), to: new Date(2023, 0, 10) }}
      fromDate={new Date(2023, 0, 1)}
      toDate={new Date(2023, 11, 31)}
      locale={navigator.language}
      weekStartsOn={1}
    />
  );
};

export default CalendarExample;
```

## Anatomy

All parts of the Calendar component can be imported and composed as required.

```typescript jsx
<Calendar
  mode="single"
  selected={new Date()}
  onSelect={(date) => alert(\`Selected date: \${date}\`)}
  showOutsideDays={true}
  className="custom-calendar"
  classNames={{
    day: 'custom-day',
    selected: 'custom-selected',
    today: 'custom-today',
  }}
  components={{
    Day: ({ date, selected, ...props }) => (
      <div {...props} className={selected ? 'custom-selected' : 'custom-day'}>
        {date.getDate()}
      </div>
    ),
  }}
  disabled={{ from: new Date(2023, 0, 1), to: new Date(2023, 0, 10) }}
  fromDate={new Date(2023, 0, 1)}
  toDate={new Date(2023, 11, 31)}
  locale={navigator.language}
  weekStartsOn={1}
/>
```

## API Reference

### Calendar

The `Calendar` component serves as the main container for the date picker.

<DocsPage.PropsTable
  props={[
    {
      name: "mode",
      description: "The selection mode for the calendar",
      required: false,
      value: "'single' | 'multiple' | 'range'",
      defaultValue: "'single'",
    },
    {
      name: "selected",
      description: "The currently selected date(s)",
      required: false,
      value: "Date | Date[] | { from: Date; to: Date }",
    },
    {
      name: "onSelect",
      description: "Function to handle date selection",
      required: false,
      value: "(date: Date | Date[] | { from: Date; to: Date }) => void",
    },
    {
      name: "showOutsideDays",
      description: "Whether to show days from adjacent months",
      required: false,
      value: "boolean",
      defaultValue: "true",
    },
    {
      name: "className",
      description: "Additional class names to apply to the calendar",
      required: false,
      value: "string",
    },
    {
      name: "classNames",
      description: "Custom class names for various parts of the calendar",
      required: false,
    },
    {
      name: "components",
      description: "Custom components for various parts of the calendar",
      required: false,
    },
    {
      name: "disabled",
      description: "Dates that should be disabled",
      required: false,
      value: "Date | Date[] | { from: Date; to: Date }",
    },
    {
      name: "fromDate",
      description: "The earliest date that can be selected",
      required: false,
      value: "Date",
    },
    {
      name: "toDate",
      description: "The latest date that can be selected",
      required: false,
      value: "Date",
    },
    {
      name: "locale",
      description: "The locale to use for the calendar",
      required: false,
      value: "Locale",
    },
    {
      name: "weekStartsOn",
      description: "The day the week starts on",
      required: false,
      value: "0 | 1 | 2 | 3 | 4 | 5 | 6",
    },
  ]}
/>
