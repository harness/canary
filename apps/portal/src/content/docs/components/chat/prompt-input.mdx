...
<Aside type="tip" title="When to use PromptInput">
  Use `PromptInput` for the composer in an AI assistant surface — a textarea with tags,
  a toolbar, and a submit. For a plain form text field, use a text input. For displaying
  a sent message, use `MessageBubble`.
</Aside>
