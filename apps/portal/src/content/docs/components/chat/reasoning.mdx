...
<Aside type="tip" title="When to use Reasoning">
  Use `Reasoning` to show an AI model's in-progress thinking as a collapsible, streaming
  block, kept visibly separate from its final answer. For the answer itself, use
  `MessageBubble`. Reasoning is the model's process, never presented as settled fact.
</Aside>
