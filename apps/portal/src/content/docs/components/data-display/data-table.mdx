...
<Aside type="tip" title="When to use DataTable">
  Use `DataTable` for interactive tabular data — sortable columns, row selection,
  column filtering or pinning, nested or expandable rows (it wraps TanStack Table).
  For static, read-only display, use `Table` — it's lighter and has no TanStack
  dependency. For a list of heterogeneous items rather than columns, use `StackedList`.
  Remember to wire the empty, loading, and error states yourself — DataTable doesn't
  render them for you.
</Aside>
