...
// Define columns with enableHiding
const columns = [
  {
    accessorKey: "name",
    header: "Name",
          ...
          {columns.map((column) => (
            <DropdownMenu.CheckboxItem
              ...
              onCheckedChange={(checked) =>
                handleColumnToggle(column.accessorKey, checked)
              }
            />
          ))}
        </DropdownMenu.Content>
...
<Aside type="tip">
  The `visibleColumns` prop works with columns that have `enableHiding: true`.
  This allows you to dynamically control which columns are displayed, perfect
  for user preferences and localStorage persistence.
</Aside>
