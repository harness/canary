...
Use `enableStickyHeader` to keep table header rows pinned while the table body scrolls. The consumer must constrain the table height, for example with a fixed-height wrapper or a height class on the table container.
...
```tsx
<div className="h-[280px]">
  <DataTable columns={columns} data={data} className="h-full" enableStickyHeader />
</div>
```
...
<DocsPage.PropsTable
  props={[
    ...
    },
    {
      ...
      description:
        "Keep table header rows pinned to the top of the table's scroll container. Consumers must constrain the table height for vertical scrolling.",
      required: false,
      value: "boolean",
      defaultValue: "false",
    },
