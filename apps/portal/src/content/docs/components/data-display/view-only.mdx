...
<Aside type="tip" title="When to use ViewOnly">
  Use `ViewOnly` to display read-only label-value pairs — a resource's details, a
  run's metadata, a config summary. For editable fields, use `Form` and its inputs. For
  columnar data across many rows, use `Table`.
</Aside>
