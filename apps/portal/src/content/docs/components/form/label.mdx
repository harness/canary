---
title: Label
description: Label component
beta: true
---

The `Label` component is a customizable label for form elements. It includes an optional required indicator and follows consistent styling across the design system.

import { DocsPage } from "@/components/docs-page";
import { Aside } from "@astrojs/starlight/components";

<Aside type="tip" title="When to use Label">
  Use `Label` to caption a form control accessibly. Most inputs accept a `label` prop
  directly — reach for the standalone `Label` only when composing a custom control that
  doesn't. For helper or error text, use `Caption`.
</Aside>

<DocsPage.ComponentExample
  client:only
  code={`
    <div className="flex flex-col gap-cn-md">
      <Label htmlFor="name">Name</Label>
      <TextInput id="name" type="text" />
    </div>
`}
/>

## Usage

```typescript jsx
import { Label } from '@harnessio/ui/components'

// ...

return (
  <>
    <Label htmlFor="username">Username</Label>
    <TextInput id="username" type="text" />
  </>
)
```

## Optional

<DocsPage.ComponentExample
  client:only
  code={`
    <div className="flex flex-col gap-cn-md">
      <Label htmlFor="email" optional>Email</Label>
      <TextInput id="email" type="email" />
    </div>
  `}
/>

## With tooltip (Informer)

The `Label` component supports displaying a tooltip with additional information.
Use the `tooltipContent` prop to provide the tooltip content, and pass additional configuration via the `tooltipProps` prop.

<DocsPage.ComponentExample
  client:only
  code={`
  <Layout.Vertical gap="xl">
    <div className="flex flex-col gap-cn-md">
      <Label
        htmlFor="2"
        tooltipContent="Content here"
      >
        Default icon
      </Label>
      <TextInput id="2" type="text" />
    </div>
    <div className="flex flex-col gap-cn-md">
      <Label
        htmlFor="3"
        tooltipContent="Content here"
        tooltipProps={{
          disabled: false,
          iconProps: {
            name: 'warning-triangle',
            color: 'warning'
          },
        }}
      >
        Custom icon
      </Label>
      <TextInput id="3" type="text" />
    </div>
  </Layout.Vertical>
`}
/>

## Suffix

<DocsPage.ComponentExample
  client:only
  code={`
    <div className="flex flex-col gap-cn-md">
      <Label
        htmlFor="2"
        tooltipContent="Content here"
        suffix={
          <Button size="2xs" iconOnly>
            <IconV2 name="plus" skipSize />
          </Button>
        }
      >
        Description
      </Label>
      <TextInput id="2" type="text" />
    </div>
  `}
/>

## API Reference

```typescript
interface IconWithTooltipProps extends Omit<TooltipProps, "children"> {
  className?: string;
  /**
   * If disabled, the tooltip will not be shown
   */
  disabled?: boolean;
  iconProps?: IconPropsV2;
}
```

<DocsPage.PropsTable
  props={[
    {
      name: "htmlFor",
      description: "The id of the form control this label is associated with.",
      required: false,
      value: "string",
    },
    {
      name: "optional",
      description: "Indicates if the associated form field is optional.",
      defaultValue: "false",
      required: false,
      value: "boolean",
    },
    {
      name: "className",
      description: "Additional CSS classes to apply to the label.",
      required: false,
      value: "string",
    },
    {
      name: "children",
      description: "The content of the label.",
      required: true,
      value: "ReactNode",
    },
    {
      name: "tooltipContent",
      description: "Content to display in the tooltip.",
      required: false,
      value: "Exclude<ReactNode, boolean | null | undefined>",
    },
    {
      name: "tooltipProps",
      description: "Additional props for configuring the tooltip.",
      required: false,
      value: "IconWithTooltipProps",
    },
    {
      name: "suffix",
      description:
        "Customized with a suffix element that appears on the right side of the label.",
      required: false,
      value: "ReactNode",
    },
  ]}
/>
