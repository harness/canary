...
<Aside type="tip" title="When to use MultiSelect">
  Use `MultiSelect` to choose *several values* from a set — tags, labels, scopes,
  members — shown back as removable chips. For exactly one value, use `Select`. For a
  small fixed set where every option should be visible, use a `Checkbox` group. To
  trigger actions, use `DropdownMenu`.
</Aside>
