...
<Aside type="tip" title="When to use NumberInput">
  Use `NumberInput` for a numeric value with stepping and validation — a timeout, a
  replica count, a retry limit. For free text, use a text input. For a bounded range the
  user drags, use `Slider`.
</Aside>
