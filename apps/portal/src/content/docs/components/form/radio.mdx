...
<Aside type="tip" title="When to use Radio">
  Use `Radio` for 2–5 mutually-exclusive options that benefit from being visible at
  once — a connector type, a deployment strategy, a repository visibility level —
  submitted with the form. For many options or a long list, use `Select`. For
  non-exclusive choices, use `Checkbox`. For an instant-apply mode switch, use
  `Switch` or `ToggleGroup`.
</Aside>
