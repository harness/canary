...
<Aside type="tip" title="When to use Radio">
  ...
  once — a visibility level, a plan tier — submitted with the form. For many options or
  a long list, use `Select`. For non-exclusive choices, use `Checkbox`. For an
  instant-apply mode switch, use `Switch` or `ToggleGroup`.
</Aside>
