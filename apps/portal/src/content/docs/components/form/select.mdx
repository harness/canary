...
The `Select` component provides a dropdown interface for selecting a single value from a list of options. It supports both controlled and uncontrolled modes, clearable selections, async loading states, search functionality, and integrates seamlessly with form validation.
...
<Select
  ...
  clearable                 // [OPTIONAL] show clear button when selected
  onClear={onClear}         // [OPTIONAL] callback when selection is cleared
  onOpen={onOpen}           // [OPTIONAL] callback when dropdown opens
...
- When `clearable` is true, selected values can be reset to `undefined`
