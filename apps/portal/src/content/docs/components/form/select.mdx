...
<Aside type="tip" title="When to use Select">
  Use `Select` to choose *one value* from a known set inside a form — a status, an
  environment, an owner. For *multiple* values, use `MultiSelect`. For 2–5 visible
  mutually-exclusive options, use `Radio`. To trigger *actions* or commands rather than
  pick a value, use `DropdownMenu` — value-selection and actions are different jobs.
</Aside>
