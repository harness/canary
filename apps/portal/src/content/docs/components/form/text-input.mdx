...
<Aside type="tip" title="When to use Text Input">
  Use a text input when the user types a free value — a name, a URL, a search term.
  For choosing from known options, use `Select` or `MultiSelect`. For a boolean, use
  `Switch` or `Checkbox`. For a date, use `Calendar`. Pair it with a label (a
  placeholder is not a label) and show validation errors as text, not color alone.
</Aside>
