...
<Aside type="tip" title="When to use Textarea">
  Use `Textarea` for multi-line free text — a description, a commit message, a YAML
  snippet pasted by hand. For a single line, use a text input. For @-mentions, use the
  mention textarea. For rendered markdown, use `MarkdownViewer`.
</Aside>
