...
<Aside type="tip" title="When to use DiffViewer">
  Use `DiffViewer` to compare two versions of code or text — a PR diff, a config change,
  a suggested edit. For rendering a single document, use `MarkdownViewer`. Choose Split
  (side-by-side) for wide screens and structural changes, Unified (inline) for narrow
  columns or small edits.
</Aside>
...
Pass `diffViewMode={DiffModeEnum.Unified}` to render the diff inline instead of side-by-side.
...
The DiffViewer supports two display modes via the `diffViewMode` prop. It defaults to **Split mode** when no mode is provided.

### Split Mode (Side-by-Side)

`diffViewMode={DiffModeEnum.Split}` (default)
...
- Shows old and new code stacked inline in a single column
- Ideal for narrow layouts or focused review of small changes
- Easier to follow short, sequential edits
- Works well in side panels and mobile viewports
