...
<Aside type="tip" title="When to use MarkdownViewer">
  Use `MarkdownViewer` to safely render authored markdown — a README, a description,
  release notes, a PR body — with syntax highlighting and sanitization. To compare two
  versions, use `DiffViewer`. For a plain code snippet, use a code block. Don't use it
  to fake editable app UI — for real input use `Form` and its inputs.
</Aside>
