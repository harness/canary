...
Apply muted text color to markdown content using the `muted` prop. This is useful when the viewer appears in a secondary or subdued context, such as a sidebar or metadata panel.
...
<MarkdownViewer
    withBorder
    source={\`## Default
This markdown uses the **default** text color.
Emphasis and other styling remain fully visible.\`}
/>
<MarkdownViewer
    muted
    withBorder
    source={\`## Muted
This markdown uses the **muted** text color.
Emphasis and other styling remain fully visible.\`}
/>
</Layout.Flex>`}
/>
...
<DocsPage.PropsTable
  props={[
    ...
    },
    {
      name: "muted",
      description:
        "Apply muted text color (text-cn-3) to all markdown content. Use when the viewer appears in a secondary or subdued context.",
      required: false,
      defaultValue: "false",
      value: "boolean",
    },
