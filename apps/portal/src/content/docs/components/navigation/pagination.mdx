...
<Aside type="tip" title="When to use Pagination">
  Use `Pagination` to page through a bounded list of results — pipeline runs, audit
  entries, search results — when the user needs to jump to a specific page. For
  continuous scroll through an open-ended feed, prefer incremental loading. `Table`,
  `DataTable`, and `StackedList` accept pagination props directly.
</Aside>
