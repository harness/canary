...
<Aside type="tip" title="When to use Sidebar">
  Use `Sidebar` for the product's top-level navigation between sections — a persistent,
  collapsible left rail. For chrome and context across the top, use `Topbar`. For
  where-am-I hierarchy, use `Breadcrumb`. For switching views *within* a page, use
  `Tabs`. For a menu of actions, use `DropdownMenu`. Always mark the active item.
</Aside>
