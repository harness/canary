...
<Aside type="tip" title="When to use TreeView">
  Use `TreeView` to display and navigate hierarchical data that isn't a filesystem — an
  org/project/resource hierarchy, a nested config. For a file/folder tree specifically,
  use `FileExplorer`. For a flat list of entities, use `StackedList`.
</Aside>
