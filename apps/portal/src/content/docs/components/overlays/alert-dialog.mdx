...
<Aside type="tip" title="When to use AlertDialog">
  Use `AlertDialog` when the action can't be cheaply undone — delete, force-push,
  rollback, revoke — and the user must explicitly accept or cancel. For a neutral
  task or a form, use `Dialog`. For a reversible action, skip the overlay
  entirely: act, and offer an undo via `Toast`. For a passive, non-blocking
  warning, use `Alert`.
</Aside>
...
<DocsPage.ComponentExample
  client:only
  code={`<div className="flex items-center justify-center gap-cn-md">
