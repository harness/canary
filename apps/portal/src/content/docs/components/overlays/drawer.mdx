...
<Aside type="tip" title="When to use Drawer">
  Use `Drawer` when the user works on something while keeping the underlying
  context visible — a row's detail/edit panel, a create/edit sub-flow, or a
  multi-step wizard. It supports nesting and dual-pane layouts. For a short
  blocking decision, use `Dialog`. For a destructive confirm, use `AlertDialog`.
  For a single-purpose lightweight panel with no sub-flow, use `Sheet`. For a tiny
  contextual menu, use `Popover` or `DropdownMenu`.
</Aside>
