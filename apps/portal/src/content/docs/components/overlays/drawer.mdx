...
The [HeaderV2](/components/overlays/drawer/header-v2/) and [Dual Pane](/components/overlays/drawer/dual-pane/) layouts each have their own page.
...
## Structured header (props API)

`Drawer.Header` accepts a structured, opinionated set of props that consolidate tagline/breadcrumb, icon/logo, title, description, actions, metadata (children), and tabs into a single standardized layout with optional loading state. Pass a `title` prop to opt into this layout; pass children (`Drawer.Title`, `Drawer.Tagline`, `Drawer.Description`) to use the legacy compositional layout instead. Both are supported on the same component so existing usage keeps working.

> **Deprecated:** `Drawer.HeaderV2` is now an alias of `Drawer.Header` with these props. Prefer `Drawer.Header` for new usage.
...
Each entry in the `tabs` array accepts the following fields.
...
<DocsPage.PropsTable
  props={[
    ...
    {
      name: "counter",
      description: "Counter badge value to display after the label.",
      required: false,
      value: "number",
      defaultValue: "undefined",
    },
    {
      name: "badge",
      description:
        "Optional status badge rendered after the label. Pass a string for a simple badge, or StatusBadge props (`variant`, `theme`, `content`, etc.).",
      required: false,
    ...
    },
    {
      name: "exact",
      description:
        "When true, matches the route exactly. Only applies to navigation tabs.",
      required: false,
      value: "boolean",
      defaultValue: "false",
    },
