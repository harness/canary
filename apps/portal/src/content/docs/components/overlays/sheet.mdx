...
<Aside type="tip" title="When to use Sheet">
  Use `Sheet` for a simple edge-anchored panel with one job and no sub-flow — a
  quick filter set, a compact detail peek, a narrow or compact context. For
  multi-step, dual-pane, or nested panels, use `Drawer`. For a blocking decision,
  use `Dialog`. For rich content anchored to a trigger, use `Popover`.
</Aside>
