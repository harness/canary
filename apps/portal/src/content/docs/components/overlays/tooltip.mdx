...
<Aside type="tip" title="When to use Tooltip">
  Use `Tooltip` for a brief label, a name for an icon-only control, or a
  non-essential hint shown on hover or focus. For anything interactive, or for
  information the user genuinely needs, use `Popover` — never hide essential
  information behind a hover. For a list of actions, use `DropdownMenu`.
</Aside>
