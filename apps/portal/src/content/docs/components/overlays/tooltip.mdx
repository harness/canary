...
The tooltip supports `default` and `inverse` variants. The inverse variant provides high-contrast appearance.
  ...
  code={`
    <TooltipProvider>
      <ButtonLayout className="justify-center">
        ...
        >
          <Button variant="secondary">Default Variant</Button>
        </Tooltip>
        ...
        >
          <Button variant="secondary">Inverse Variant</Button>
        </Tooltip>
      </ButtonLayout>
    </TooltipProvider>
`}
...
<DocsPage.PropsTable
  props={[
    ...
    {
      name: "sideOffset",
      description: "Distance in pixels from the trigger element. Automatically increases to 6px when arrow is hidden.",
      required: false,
      value: "number",
