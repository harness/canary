...
interface FlowConfig {
  ...
  steps: Record<string, StepConfig>; // Card definitions (right pane)
...
<Aside type="note">
  ...
  Multiple steps can belong to the same step group (e.g., for multi-card
  analysis sequences). The `next` field defines the default routing — cards
  advance automatically unless an explicit destination is passed to `complete()`
  or `skip()`.
</Aside>
