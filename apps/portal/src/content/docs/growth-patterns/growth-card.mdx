  ...
  code={`() => {
    ...
    return (
      <Card.Root size="sm" interactive={false} className="cn-growth-card w-full">
        <Card.Title>You're almost done with setup</Card.Title>
        <Card.Content>Just 2 steps left: connect a source and pick a destination.</Card.Content>
        <div className="cn-growth-card-actions flex justify-end gap-cn-xs">
          <Button variant="ghost" size="xs" onClick={() => setVisible(false)}>
            Dismiss
          </Button>
          <Button variant="ai" size="xs" className="cn-growth-card-cta">
            Finish setup
          </Button>
        </div>
      </Card.Root>
...
<DocsPage.ComponentExample
  ...
  code={`() => {
    const CARDS = [
      {
        title: "You're almost done with setup",
        content: "Just 2 steps left: connect a source and pick a destination.",
        cta: "Finish setup",
      },
      {
    ...
    if (queue.length === 0) {
      return (
        <div className="flex items-center gap-cn-xs">
          <Text color="foreground-3">All growth boxes dismissed — don't show the same message after dismissal</Text>
...
/* Primary CTA border: border/brand @ 16% → set/purple/primary @ 87%, 135deg */
.cn-growth-card-cta {
  background-image:
    var(--cn-set-ai-outline-bg-gradient),
    linear-gradient(
      135deg,
...
<Card.Root size="sm" interactive={false} className="cn-growth-card">
  ...
  <div className="flex justify-end gap-cn-xs cn-growth-card-actions">
    ...
    <Button variant="ai" size="xs" className="cn-growth-card-cta" onClick={onPrimary}>
      Finish setup
    </Button>
  </div>
</Card.Root>
...
| Primary action | `Button` (`variant="ai"`, `className="cn-growth-card-cta"`, `size="xs"`) | Main CTA. Growth-specific border: `border/brand` @ 16% → `set/purple/primary` @ 87% (135deg)     |
...
- **Constrain width.** Growth boxes are narrow by design — default width is `220px`, set via `cn-growth-card` / `cn-growth-card-stack`.
