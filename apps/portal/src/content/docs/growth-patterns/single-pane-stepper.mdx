...
interface FlowConfig {
  ...
  steps: Record<string, StepConfig>; // Card definitions
...
<Aside type="note">
  Each step must be associated with a step group via `step: 'group-id'`.
  Multiple steps can belong to the same step group. The `next` field defines the
  default routing — cards advance automatically unless an explicit destination
  is passed to `complete()` or `skip()`.
</Aside>
...
<DocsPage.PropsTable
  props={[
    ...
    },
    {
      name: "showStepBadge",
      description:
        "Renders a 'Step {n}/{total}' pill badge next to each step's title",
      required: false,
      value: "boolean",
      defaultValue: "false",
    },
    {
      name: "flat",
      description:
        "Renders each step group's steps as flat top-level Stepper.Step items (plain straight connector, no branch/StepGroup nesting) instead of the default nested/branch-connector layout",
      required: false,
      value: "boolean",
      defaultValue: "false",
    },
