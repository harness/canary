...
The `SinglePaneStepper` is a single-column flow component for multi-step guided processes. It combines a vertical stepper timeline with cards rendered inline within each substep — all in one scrollable column.
...
Substeps render distinct states in the timeline — completed (green check), skipped (arrow), and
errored (red). A step whose failed substep is retried and completed shows as completed overall, with
the failed substep still marked in the timeline. This example settles into a fully resolved flow on
load.
...
<Aside type="note">
  ...
  substeps can belong to the same step. The `next` field defines the default
  routing — cards advance automatically unless an explicit destination is passed
  to `complete()` or `skip()`.
</Aside>
...
- **`error(nextSubStepId?)`** — Mark the card as errored. Pass a `nextSubStepId` to advance while keeping the substep red in the timeline (error-and-continue).
- **`skip(nextSubStepId?)`** — Mark the card as skipped and advance.
...
## Substeps and Progressive Disclosure

The stepper timeline reveals content progressively. Top-level steps only appear once reached — completed, active, and error states are visible; upcoming steps stay hidden until the flow advances to them.

Within each step, only visited substeps appear in the timeline. Cards render inline beneath their substep as the user progresses. Unvisited substeps do not show as upcoming placeholders — each substep and its card appear only after the user reaches it. The `next` field in your flow config still drives default routing when cards complete, but does not preview future substeps in the timeline.
...
Users can click completed substeps in the timeline to scroll back to earlier cards. When navigating backward to reactivate a card, the reactivation guard shows a confirmation dialog:
...
A card rendered inline within a substep. Shows status indicator, title, optional description, and content.
...
<DocsPage.PropsTable
  props={[
    ...
    },
    {
      name: "error",
      description:
        "Mark the card as errored. Pass nextSubStepId for error-and-continue.",
      required: true,
