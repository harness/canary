...
export function useCapability<TArgs = any, TResult = any>(config: CapabilityConfig<TArgs, TResult>): void {
  ...
  useEffect(() => {
    // Register handler
    if (config.execute) {
      runtime.capabilityRegistry.registerHandler(config.name, { execute: config.execute }, config.strategy)
    }

    console.log('Registering renderer for capability', config.name, config.render)
    // Register renderer
    if (config.render) {
