...
export function checkInstance(
  ...
  if (snapshot.isFromLibrary === false && !hasVerifiedKeyIdentity) {
    ...
    snapshot.isFromLibrary === null &&
    opts.treatMissingLibraryFlagAs === "warn"
  ) {
    findings.push({
      ...
      catalogId: entry.id,
      message:
        "Could not determine whether this instance comes from the team library.",
    });
  }
  ...
  for (const [rawName, rawValue] of Object.entries(snapshot.properties)) {
    ...
    if (shared) {
      ...
      if (shared.type === "enum" && shared.values) {
        ...
        if (!ok) {
          const suggestion = suggestClosestValue(
            String(normalized),
            shared.values,
          );
    ...
    const heuristicType =
      typeof rawValue === "boolean" ? "designOnly" : "shared";
    findings.push({
...
function unmappedResult(
  ...
  const proposeDefaults: Partial<ProposalDraft> = {
    ...
    attemptedWorkaround:
      "Checked componentKey, component-set key, and figmaNames.",
