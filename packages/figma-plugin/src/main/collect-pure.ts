 ...
 */
const DETACHABLE_NODE_TYPES = new Set(['COMPONENT', 'COMPONENT_SET', 'FRAME', 'GROUP'])
...
export function componentPropertyToPrimitive(
  ...
  if (typeof value === 'object' && value !== null && 'value' in value) {
    ...
    if (typeof inner === 'string' || typeof inner === 'boolean' || typeof inner === 'number') {
      return inner
    }
  }
  return String(value)
...
export function toInstanceSnapshot(input: {
  ...
  for (const [name, raw] of Object.entries(input.componentProperties)) {
    const prim = componentPropertyToPrimitive(raw as string | boolean | number | object | null | undefined)
...
export function normalizeCatalogNames(names: readonly string[]): ReadonlySet<string> {
