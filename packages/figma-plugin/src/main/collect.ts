...
export type CollectableNode = {
  ...
  componentProperties?: Record<string, { value: unknown; type: string }> | null
 ...
 */
export async function readMainComponent(instance: CollectableNode): Promise<MainComponentLike | null> {
...
async function snapshotFromInstance(
  ...
  const isFromLibrary = main && typeof main.remote === 'boolean' ? Boolean(main.remote) : null
  ...
  return toInstanceSnapshot({
    ...
    componentProperties,
    propertyDefinitions,
    childNames: (instance.children ?? []).map(child => child.name),
    nodeType: 'INSTANCE',
    parentNodeId
  })
}
...
function snapshotFromCatalogComponent(
  ...
  return toInstanceSnapshot({
    ...
    propertyDefinitions,
    childNames: (component.children ?? []).map(child => child.name),
...
export async function collectFromNodes(
  ...
  const visit = async (node: CollectableNode) => {
    ...
    if (node.type === 'COMPONENT') {
      const componentSet = node.parent?.type === 'COMPONENT_SET' ? node.parent : null
      if ((node.key && catalogKeys.has(node.key)) || (componentSet?.key && catalogKeys.has(componentSet.key))) {
        scanned += 1
    ...
    if (isDetachableNodeType(node.type) && looksLikeCatalogComponent(node.name, catalogNames)) {
      scanned += 1
      snapshots.push(
        toDetachedSnapshot({
          nodeId: node.id,
...
export async function collectFromSelection(
  ...
  return collectFromNodes(figma.currentPage.selection as unknown as readonly CollectableNode[], cap, onProgress, opts)
}
...
export async function collectFromPage(
  ...
  await loadCurrentPage()
  return collectFromNodes(figma.currentPage.children as unknown as readonly CollectableNode[], cap, onProgress, opts)
}
