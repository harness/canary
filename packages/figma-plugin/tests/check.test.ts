...
describe("checkInstance", () => {
  ...
  it("emits DESIGN_ONLY_OK for icon boolean helpers", () => {
    const result = checkInstance(
      snap({
        properties: {
          variant: "primary",
    ...
    const designOnly = result.findings.filter(
      (f) => f.code === "DESIGN_ONLY_OK",
    );
...
describe("unmapped instances", () => {
  ...
  it("drops unmapped parts nested inside a reported component", () => {
    const parent = snap({ nodeId: "1:1", nodeName: "Button" });
    const part = snap({
      nodeId: "1:2",
      nodeName: "prefix",
      mainComponentName: "❖IconSlot",
