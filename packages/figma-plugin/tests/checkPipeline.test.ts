...
function iconInstance(id: string, name: string, children: CollectableNode[] = []): CollectableNode {
  return {
    id,
    name,
    type: 'INSTANCE',
...
function buttonParts(): CollectableNode[] {
  return [
    iconInstance('1:1510', 'prefix', [iconInstance('I1:1510;23:1408', 'icon-color')]),
    iconInstance('1:1512', 'suffix', [iconInstance('I1:1512;3250:96', 'icon-color')])
  ]
}
...
describe('Button definition selected in the source library', () => {
  it('checks the Button variant once and folds its icon slots into it', async () => {
    ...
    const sourceVariant = {
      ...
      children: [
        ...
        { id: '28553:15464', name: 'text', type: 'TEXT' },
    ...
    expect(report.instances.map(instance => instance.snapshot.nodeName)).not.toEqual(
  ...
  it('audits variants inside a selected cataloged component set', async () => {
    ...
    const variants = ['primary', 'secondary'].map((variant, index) => ({
      ...
      children: [
        iconInstance(`28553:${15563 + index}`, 'prefix'),
        { id: `28553:${15663 + index}`, name: 'text', type: 'TEXT' }
      ]
    })) as CollectableNode[]
    ...
    expect(report.instances.every(instance => instance.status === 'checked')).toBe(true)
    expect(report.instances.map(instance => instance.snapshot.nodeName)).not.toContain('prefix')
    expect(report.findings.some(finding => finding.code === 'FAIL_DETACHED')).toBe(false)
    expect(report.summary).toMatchObject({
      pass: 2,
      fail: 0,
      unmapped: 0,
...
describe('detach → make component → Check selection', () => {
  it('returns one failure about the button, not its icon parts', async () => {
    const report = await check([{ id: '1:1500', name: 'Button', type: 'COMPONENT', children: buttonParts() }])
...
describe('genuine library instance', () => {
  function libraryButton(props: Record<string, { value: unknown; type: string }>): CollectableNode {
    return {
      id: '1:2000',
      name: 'Button',
      type: 'INSTANCE',
  ...
  it('accepts a local source-library instance matched by its published set key', async () => {
    ...
    const sourceLibraryButton: CollectableNode = {
      ...
      name: '❖Button/Md/Text',
      type: 'INSTANCE',
      children: [{ id: '30044:12149', name: 'text', type: 'TEXT' }],
      componentProperties: {
