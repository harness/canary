...
export const ChatDiffViewer = ({ data, mode = 4, lang = 'go', fileName }: ChatDiffViewerProps) => {
  ...
  return (
    <div className="mr-7 mt-3 flex flex-col rounded-md border border-cn-2 bg-cn-1">
      {fileName && (
        <span className="rounded-[inherit] border-b border-cn-2 bg-cn-2 p-4 text-2 font-medium text-cn-1">
          {fileName}
        </span>
      )}
      {diffFileInstance && (
