...
interface FilterTriggerProps<T extends object> {
  ...
  buttonLabel?: string
  onChange: (option: FilterOption<T>) => void
  onReset?: () => void
}
...
const FilterSelect = <T extends object>({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="flex items-center gap-x-1.5">{displayLabel}</DropdownMenuTrigger>
      <DropdownMenuContent className="min-w-[224px] p-0" align={dropdownAlign}>
        ...
        <div className="p-1">
          {filteredBySearchOptions.map(option => (
            <DropdownMenuItem key={option.value as string} onSelect={() => onChange(option)}>
              {option.label}
            </DropdownMenuItem>
          ))}
