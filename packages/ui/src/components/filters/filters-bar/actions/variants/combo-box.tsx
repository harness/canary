...
export default function ComboBox({
  ...
  const renderContent = () => {
    if (isLoading) {
      return <Command.Loading className="px-2 py-4 text-sm text-cn-foreground-5">{loadingMessage}</Command.Loading>
    }
      ...
      return (
        ...
        >
          <div className="mx-2 flex size-4 items-center">
            {value === selectedFilterValue && <Icon name="tick" size={12} className="text-cn-foreground-8" />}
          </div>
          {label}
        </Command.Item>
      )
