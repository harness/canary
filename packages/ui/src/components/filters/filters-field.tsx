...
const FilterFieldInternal = <T extends string, V extends FilterValueTypes, CustomValue = Record<string, unknown>>({
  ...
  switch (filterOption.type) {
    ...
    case FilterFieldTypes.Checkbox: {
      ...
      return (
        <Label
          className="rounded-3 bg-cn-gray-soft border-cn-gray-surface cursor-pointer border px-2.5 py-1.5 [&>.cn-label-text]:flex [&>.cn-label-text]:items-center [&>.cn-label-text]:gap-x-2.5"
          htmlFor={checkboxId}
        >
          <Checkbox
            id={checkboxId}
