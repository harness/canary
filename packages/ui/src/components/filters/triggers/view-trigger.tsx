        ...
        {!!layoutOptions?.length && (
          <>
            <div className="px-2 py-2.5">
              <span className="text-13 leading-none text-cn-foreground-7">Layout</span>
            </div>
            {layoutOptions.map(option => (
              <DropdownMenu.Item key={option.value} onSelect={() => onLayoutChange?.(option.value)} className="gap-x-2">
                ...
                {currentLayout === option.value && (
                  <Icon name="tick" size={12} className="ml-auto text-cn-foreground-8" />
                )}
              </DropdownMenu.Item>
            ))}
          </>
          ...
          <>
            ...
            <div className="px-2 py-1.5">
              <div className="flex items-center justify-between">
                <span className="text-13 leading-none text-cn-foreground-7">Saved views</span>
                <button
            ...
            {savedViews.map(view => (
              <DropdownMenu.Item key={view.id} className="gap-x-2" onSelect={() => onViewSelect(view)}>
                <span>{view.name}</span>
                {currentView?.id === view.id && <Icon name="tick" size={12} className="ml-auto text-cn-foreground-8" />}
              </DropdownMenu.Item>
            ))}
          </>
        )}
