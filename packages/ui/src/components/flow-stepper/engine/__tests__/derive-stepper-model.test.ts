...
describe('deriveStepperModel', () => {
  ...
  test('recovered error (error then a later substep completed) marks the STEP completed, substep stays error', () => {
    // a errored, but the flow continued to b which completed — the step recovered.
    const history: CardEntry[] = [
      { subStepId: 'a', status: 'error', stateSnapshot: {} },
      { subStepId: 'b', status: 'completed', stateSnapshot: {} }
    ]
    const model = deriveStepperModel(flow, history, [], 'b')
    // Step s1 is completed (recovered), NOT error — the error is not the trailing/unresolved state.
    expect(model[0].state).toBe('completed')
    // The individual errored substep still renders red in the timeline.
    expect(model[0].visited).toEqual([
      { subStepId: 'a', state: 'error' },
      { subStepId: 'b', state: 'completed' }
    ])
    // No indeterminate "..." placeholder on a resolved step.
    expect(model[0].showIndeterminate).toBe(false)
  })

  test('trailing/unresolved error still marks the step error', () => {
    // a completed, b errored and is the last visited substep — the step is stopped on the error.
    const history: CardEntry[] = [
      { subStepId: 'a', status: 'completed', stateSnapshot: {} },
      { subStepId: 'b', status: 'error', stateSnapshot: {} }
    ]
    const model = deriveStepperModel(flow, history, [], 'b')
    expect(model[0].state).toBe('error')
  })
