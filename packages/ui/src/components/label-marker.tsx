...
export const LabelMarker: FC<LabelMarkerProps> = ({
  ...
  return (
    <div className={cn('flex max-w-full items-center gap-2 opa relative', className)}>
      <span
        className={cn(
          'grid text-12 leading-4 h-5 py-px px-2 items-center font-medium rounded overflow-hidden w-fit',
          LabelMarkerColor[color],
      ...
      {isWithDeleteButton && (
        <Button
          ...
          className={cn('absolute right-1 top-0.5 max-h-4 min-h-4 min-w-4 max-w-4 px-0', deleteButtonColor[color])}
          variant="custom"
          onClick={onDelete}
        >
