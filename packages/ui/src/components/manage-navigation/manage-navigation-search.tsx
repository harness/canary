  ...
  return (
    <Popover.Root open={isSearchDialogOpen} onOpenChange={setSearchDialogOpen}>
      <Popover.Trigger asChild>
        <SearchBox.Root
          className="w-full"
          inputClassName="h-9 placeholder:text-cn-foreground-5"
          ref={inputRef}
        ...
        <ScrollArea className={cn('relative max-h-[50vh]', countFilteredItems > 10 && 'h-[404px]')}>
          <div className="px-1 pb-2 pt-1">
            ...
            {countFilteredItems === 0 ? (
              <Text className="block w-full px-2 py-4 text-cn-foreground-5">No results found</Text>
            ) : (
              filteredItems.map((category, index) => (
                ...
                >
                  <Text className="inline-block px-2 leading-none text-cn-foreground-7" size={1}>
                    {category.title}
                  </Text>
                    ...
                    {category.items.map(item => (
                      ...
                      >
                        <div className="flex w-full items-center gap-x-2">
                          <Text className="truncate leading-tight text-cn-foreground-8">{item.title}</Text>
                        </div>
                      </Button>
                    ))}
                  </div>
