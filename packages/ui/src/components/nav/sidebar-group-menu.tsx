...
export default function SidebarGroupMenu({
  ...
  const renderMenuItems = (items: ScopedNavbarItemType[]) => (
    <div className={cn('cn-sidebar-group-items', isPopover && 'cn-sidebar-group-items-popover')}>
      {items.map(item => (
    ...
    <Layout.Flex direction="column" gap="none" className={className}>
      {menuItems.map((group, index) => (
        <Fragment key={group.groupId}>
          <Sidebar.Group label={(group.title ?? '').toUpperCase()}>{renderMenuItems(group.items)}</Sidebar.Group>
