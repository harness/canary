...
>(({ className, children, viewportClassName, scrollBarClassName, ...props }, ref) => {
  ...
  return (
    <ScrollAreaPrimitive.Root ref={ref} className={cn('relative overflow-hidden flex-1', className)} {...props}>
      ...
      <ScrollBar className={isScrolling ? 'opacity-100' : undefined} scrollBarClassName={scrollBarClassName} />
...
>(({ className, orientation = 'vertical', scrollBarClassName, ...props }, ref) => (
  ...
  >
    <ScrollAreaPrimitive.ScrollAreaThumb
      className={cn('relative flex-1 rounded-full bg-background-9', scrollBarClassName)}
    />
  </ScrollAreaPrimitive.ScrollAreaScrollbar>
))
