...
const sheetVariants = cva('fixed z-50 gap-4 bg-cn-1 p-cn-xl shadow-lg transition ease-in-out', {
  variants: {
    side: {
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title ref={ref} className={cn('text-cn-1 text-lg font-semibold', className)} {...props} />
))
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description ref={ref} className={cn('text-cn-3 text-sm', className)} {...props} />
))
