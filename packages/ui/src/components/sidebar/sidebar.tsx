...
const SidebarContent = forwardRef<HTMLDivElement, ComponentProps<'div'>>(({ className, children, ...props }, ref) => {
  return (
    <div
      ...
      {...props}
    >
      <ScrollArea scrollBarClassName="bg-sidebar-background-8">{children}</ScrollArea>
    </div>
  )
})
...
const SidebarGroup = forwardRef<HTMLDivElement, ComponentProps<'div'>>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      data-sidebar="group"
      className={cn('relative flex w-full min-w-0 flex-col px-3 py-2 border-sidebar-border-1', className)}
      {...props}
    />
  )
})
...
>(({ icon, text, className, active, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn('flex items-center select-none gap-x-2.5', { 'gap-x-0': !icon }, className)}
      {...props}
    >
      {icon && (
        <div
