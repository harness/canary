...
import { Layout, Skeleton, Text, type TextProps } from '@/components'
...
export const StatsPanel: FC<StatsPanelProps> = ({ data, isLoading = false, gap = '3xs', labelVariant }) => {
    ...
    <Layout.Flex wrap="wrap" gap="3xl">
      {data.map((stat, index) => (
        <Layout.Vertical gap={gap} key={index}>
          <Text variant={labelVariant} color="foreground-3">
            {stat.label}
          </Text>
          {isLoading ? (
