...
const toastVariants = cva(
  'pointer-events-auto relative flex h-12 w-full items-center justify-between overflow-hidden rounded border py-2 pl-3.5 pr-2 shadow-lg transition-all',
  {
    variants: {
      variant: {
        default: 'border-cn-2 bg-cn-3',
        destructive: 'bg-cn-danger-secondary border-transparent',
        success: 'border-cn-2 bg-cn-3',
        failed: 'border-cn-2 bg-cn-3'
      }
    },
    defaultVariants: {
      variant: 'default'
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close ref={ref} className={cn('absolute right-1 my-auto', className)} {...props} asChild>
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title ref={ref} className={cn('text-sm leading-tight text-cn-1', className)} {...props} />
))
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description ref={ref} className={cn('text-sm leading-tight text-cn-1', className)} {...props} />
))
