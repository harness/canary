...
const toastVariants = cva(
  'pointer-events-auto relative flex h-12 w-full items-center justify-between overflow-hidden rounded border py-2 pl-3.5 pr-2 shadow-lg transition-all',
  {
    variants: {
      variant: {
        default: 'border-cn-borders-1 bg-cn-background-3',
        destructive: 'border-transparent bg-background-toast-danger',
        success: 'border-cn-borders-1 bg-cn-background-3',
        failed: 'border-cn-borders-1 bg-cn-background-3'
      }
    },
    defaultVariants: {
      variant: 'default'
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn('absolute right-3 my-auto size-3 rounded-md focus:outline-none focus:ring-1', className)}
    toast-close=""
    {...props}
  >
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn('text-sm leading-tight tracking-[-0.02em] text-cn-foreground-1', className)}
    {...props}
  />
))
