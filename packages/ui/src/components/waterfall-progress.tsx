...
export const WaterfallProgress = forwardRef<HTMLDivElement, WaterfallProgressProps>(
  (
    ...
    const { start, end, leftPct, widthPct, ariaNow } = useMemo(() => {
    ...
    const track = (
      <div
        className={cn(
          'relative w-full overflow-hidden rounded-cn-full bg-cn-gray-secondary',
          // fixed height track; adjust if consumers override via className
          'h-2.5',
          className
        )}
        ref={ref}
