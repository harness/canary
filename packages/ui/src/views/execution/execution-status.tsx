...
const Badge: React.FC<ExecutionStatusProps & BadgeProps> = props => {
  const { status, duration, minimal, inConnector = false } = props
    ...
    case ExecutionState.PENDING:
      return minimal ? (
        <div className="flex items-center gap-1">
          ...
          <span className={inConnector ? 'text-cn-foreground-1' : 'text-cn-foreground-disabled'}>Pending</span>
        </div>
      ) : (
    ...
    case ExecutionState.RUNNING:
      return minimal ? (
        <div className="flex items-center gap-1">
          ...
          <span className={inConnector ? 'text-cn-foreground-1' : 'text-studio-3'}>Running</span>
        </div>
      ) : (
        <div className="flex items-center gap-1 rounded-md">
    ...
    case ExecutionState.FAILURE:
      return minimal ? (
        <div className="flex items-center gap-1">
          ...
          <span className={inConnector ? 'text-cn-foreground-1' : 'text-[#ED5E5E]'}>Failed</span>
        </div>
      ) : (
        <div className="flex items-center gap-1 rounded-md">
          <div className="flex items-center gap-1.5">
    ...
    case ExecutionState.SUCCESS:
      return minimal ? (
        <div className="flex items-center gap-1">
          ...
          <span className={inConnector ? 'text-cn-foreground-1' : 'text-cn-foreground-success'}>Success</span>
        </div>
      ) : (
        <div className="flex items-center gap-1 rounded-md">
