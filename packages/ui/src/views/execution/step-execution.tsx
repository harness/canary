  ...
  return (
    <Layout.Horizontal>
      <SearchBox.Root
        ...
        value={query}
      >
        <div className="absolute inset-y-0 right-1.5 my-auto flex h-5 w-8 items-center justify-center gap-1 rounded border border-borders-2 bg-cds-background-3">
    ...
    <Tabs.Root defaultValue={StepExecutionTab.LOG} className="size-full">
      <Layout.Vertical className="space-y-0">
        <Layout.Horizontal className="flex justify-between py-2.5 pl-5 pr-3.5">
          <Tabs.List className="h-8 w-fit gap-x-0.5 border border-borders-2 bg-cds-background-3">
            <Tabs.Trigger
              className="h-6 w-[68px] data-[state=active]:border data-[state=active]:border-borders-2 data-[state=active]:bg-cds-background-9"
