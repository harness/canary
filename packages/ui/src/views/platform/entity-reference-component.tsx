  ...
  const defaultEntityRenderer = ({ entity, isSelected, onSelect }: EntityRendererProps<T>) => {
    return (
      <StackedList.Item
        ...
        className={cn({ 'bg-cn-background-4': isSelected })}
        thumbnail={<Icon name="file" size={16} className="text-cn-foreground-5" />}
        actions={
          <Button
            variant="default"
            size="sm"
  ...
  const parentFolderRenderer = ({ parentFolder, onSelect }: ParentFolderRendererProps<S>) => {
    return (
      <StackedList.Item
        ...
        thumbnail={<Icon name="circle-arrow-top" size={16} className="text-cn-foreground-5" />}
      >
  ...
  const childFolderRenderer = ({ folder, onSelect }: ChildFolderRendererProps<F>) => {
    return (
      <StackedList.Item
        ...
        thumbnail={<Icon name="folder" size={16} className="text-cn-foreground-5" />}
      >
