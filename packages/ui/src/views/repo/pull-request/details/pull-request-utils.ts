...
export const jumpToFile = (
  ...
  // Phase 1: Estimate and scroll to approximate position
  const estimatedPosition = estimateScrollPosition(targetIndex, allDiffs, openItems)
  const scrollContainer = diffsContainerRef?.current?.closest('[data-scroll-container]') || document.documentElement

  // Trigger loading of diffs up to target if using infinite scroll
  if (onLoadMoreDiffs && targetIndex >= 20) {
    // 20 is INITIAL_LOAD_COUNT
    onLoadMoreDiffs(targetIndex + 10) // Load a bit beyond target
  }

  // Scroll to estimated position (instant)
  scrollContainer.scrollTo({ top: estimatedPosition })

  // Phase 2: Wait for content to load, then exact scroll
  const exactScrollToTarget = () => {
    ...
    // If target is found, do exact scroll
    if (outerBlockDOM && innerBlockDOM && diffDOM) {
  ...
  // Try exact scroll immediately (might work if already loaded)
